...
public class AppDatabase extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_AFTER_PHOTO = "after_photo"; // Шлях до фото після очищення
    ...
    // AppDatabase.java
    public void addUserPoints(int pointsToAdd, Context context) {
        SQLiteDatabase db = this.getWritableDatabase();

        // Замість "User" на "user_profile"
        try (Cursor cursor = db.rawQuery("SELECT points FROM user_profile WHERE profile_id = 1", null)) {
            int currentPoints = 0;
            if (cursor != null && cursor.moveToFirst()) {
                int pointsIndex = cursor.getColumnIndex("points");
                if (pointsIndex != -1) {
                    currentPoints = cursor.getInt(pointsIndex);
                } else {
                    Toast.makeText(context, "Колонка 'points' не знайдена", Toast.LENGTH_LONG).show();
                    return;
                }
            }
            ...
            db.update("user_profile", values, "profile_id = ?", new String[]{"1"});
        } catch (Exception e) {
            Toast.makeText(context, "Помилка бази даних: " + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
